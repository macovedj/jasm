...
const importBytes = fs.readFileSync('./wat2wasm/imports.wasm');
...
const execute = async () => {
  ...
  const importModule = await WebAssembly.compile(importBytes);
  const importInstance = await WebAssembly.instantiate(importModule, importObject);
  console.log(importInstance.exports.exported_func());
}
execute()